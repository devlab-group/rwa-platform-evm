...
) as { functions: Record<string, { signature: string; selector: string }> };

describe("erc7943Abi", () => {
  it.each(["setFrozenTokens", "forcedTransfer"])("encodes %s to the pinned selector", (name) => {
    const fragment = erc7943Abi.find((entry) => entry.type === "function" && entry.name === name);
    expect(fragment, `${name} missing from erc7943Abi`).toBeDefined();
    expect(toFunctionSelector(fragment!)).toBe(vectors.functions[name].selector);
  });
});
